...
struct Args {
    ...
    /// Bazel `--output_user_root` for the flycheck server. Highest
    /// precedence — overrides `$RULES_RUST_RA_OUTPUT_USER_ROOT` and the
    /// built-in default. See [`resolve_output_user_root`] for the search
    /// order and rationale.
    #[clap(long)]
...
fn run() -> Result<u8> {
    ...
    // The flycheck wrapper is always invoked under `bazel run` from one
    // of the rules_rust shims (see `bin/launcher_flycheck.sh`), so the
    // outer Bazel client has already set `--output_user_root` to the
    // rules_rust-dedicated value and exported it as
    // `RULES_RUST_RA_OUTPUT_USER_ROOT` for inner use. Reusing it here
    // means the inner `bazel build` hits the same server / cache the LSP
    // is already keeping warm — and stays insulated from the user's
    // primary `bazel build` (which lives on a different output_user_root
    // and so doesn't thrash this server's analysis cache when flycheck
    // flips `--error_format=json` / `--rustc_output_diagnostics=true`).
    //
    // Precedence: CLI flag > env var. No fallback — the launcher
    // generated by `setup` always exports the env var, and direct
    // invocations must pass --output-user-root explicitly. Silent
    // guessing here would diverge from setup's resolved value and
    // hand the user a different Bazel server / cold cache without
    // explanation.
    let output_user_root = args
        .output_user_root
        .clone()
        .or_else(|| {
            env::var("RULES_RUST_RA_OUTPUT_USER_ROOT")
                .ok()
                .filter(|s| !s.is_empty())
        ...
        .ok_or_else(|| {
            anyhow!(
                "no --output-user-root provided and $RULES_RUST_RA_OUTPUT_USER_ROOT is unset. \
                 Invoke flycheck through the launcher generated by `bazel run \
                 @rules_rust//tools/rust_analyzer:setup`, or pass --output-user-root explicitly."
            )
        })?;
